\newpage
\paragraph{Caso d'uso UC 1.3.2: Aggiunta di un widget checklistitem}
\label{UC 1.3.2: Aggiunta di un widget checklistitem}
\begin{figure}[ht]
	\centering
	\includegraphics[scale=0.60]{Usecases/img/{UC1.3.2}.png}
	\caption{Caso d'uso UC 1.3.2: Aggiunta di un widget checklistitem}
\end{figure}
\FloatBarrier
\begin{itemize}
\item\textbf{Attori}: Sviluppatore.
\item\textbf{Descrizione}: Lo sviluppatore vuole utilizzare un bottone particolare che possiede due stati: "clicked" e "unclicked".
\item\textbf{Precondizione}: Lo sviluppatore vuole utilizzare un widget checklistitem.
\item\textbf{Postcondizione}: Lo sviluppatore he generato codice eseguibile per creare un widget checklistitem con i parametri scelti.
\item\textbf{Scenario principale}:
	\begin{itemize}
		\item Lo sviluppatore vuole impostare lo stato del bottone (UC 1.3.2.1).
		\item Lo sviluppatore non imposta lo stato del bottone (UC 1.3.2.2).
		\item Lo sviluppatore vuole personalizzare il bottone (UC 1.3.2.3).
	\end{itemize}
\end{itemize}
